...
{	
	...
	//
	function deletecustomer($profile_id)
	{
	...
	function createcustomerprofile($first_name,$last_name,$email,$additional_data)
	{ 
    ...
    $this->authorizecimlib->set_data('merchantCustomerId', substr(md5(uniqid(rand(), true)), 16, 16));
		...
		echo '<p> Email: ' . $email . '</p>';
 		//send profile_id to ion_auth_model
 		$profile_id = $this->data['profileid'];
 		$this->ion_auth->update_profile_id($profile_id,$email);
		
    // Create the Payment Profile
		$this->authorizecimlib->set_data('customerProfileId', $this->data['profileid']);
		$this->authorizecimlib->set_data('billToFirstName', $first_name);
		$this->authorizecimlib->set_data('billToLastName', $last_name);
		$this->authorizecimlib->set_data('billToAddress', $additional_data['business_street_address']);
		$this->authorizecimlib->set_data('billToCity', $additional_data['business_city']);
		$this->authorizecimlib->set_data('billToState', $additional_data['business_state']);
		$this->authorizecimlib->set_data('billToZip', $additional_data['business_zip_code']);
		$this->authorizecimlib->set_data('billToCountry', 'US');
		$this->authorizecimlib->set_data('billToPhoneNumber', $additional_data['business_phone_number']);
		//$this->authorizecimlib->set_data('billToFaxNumber', '800-555-2345');
		//$this->authorizecimlib->set_data('cardNumber', '6111111111111111'); // will produce a decline
		$this->authorizecimlib->set_data('cardNumber',  $additional_data['card_number']);
		$this->authorizecimlib->set_data('expirationDate', $additional_data['card_expiration_year'], $additional_data['card_expiration_month']);
		...
		{
			//redirect('site/creditcard_declined'); 
			echo '<p> Error: ' . $this->authorizecimlib->get_error_msg() . '</p>';
			die();
		} else {
			
			$payment_profile_id = $this->data['paymentprofileid'];
 			$this->ion_auth->update_payment_profile_id($payment_profile_id,$email);
		}
		
		// Find out if it was approved or not.
		$test = $this->_validateresponse();
	...
	//
	function getcustomer($profile_id)
	{
